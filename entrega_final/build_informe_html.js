/**
 * Convierte INFORME_TECNICO_FINAL.md en un HTML imprimible (A4) listo para
 * exportar a PDF desde el navegador (Cmd/Ctrl+P → Guardar como PDF).
 *
 * Sin dependencias externas. Uso:
 *   node build_informe_html.js
 *   # genera INFORME_TECNICO_FINAL.html en la misma carpeta
 */
const fs = require("fs");
const path = require("path");

const SRC = path.join(__dirname, "INFORME_TECNICO_FINAL.md");
const OUT = path.join(__dirname, "INFORME_TECNICO_FINAL.html");

function escapeHtml(s) {
  return s
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
}

function inline(text) {
  // Proteger imágenes ![alt](src) antes de escapar
  const imgs = [];
  text = text.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_, alt, src) => {
    imgs.push({ alt, src });
    return "\u0000IMG" + (imgs.length - 1) + "\u0000";
  });
  // Proteger asteriscos escapados
  text = text.replace(/\\\*/g, "\u0000ESC_AST\u0000");
  text = escapeHtml(text);
  // Proteger código en línea
  const codes = [];
  text = text.replace(/`([^`]+)`/g, (_, c) => {
    codes.push(c);
    return "\u0000CODE" + (codes.length - 1) + "\u0000";
  });
  // Negrita y cursiva
  text = text.replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>");
  text = text.replace(/\*([^*]+)\*/g, "<em>$1</em>");
  // Restaurar código en línea
  text = text.replace(/\u0000CODE(\d+)\u0000/g, (_, i) => "<code>" + codes[+i] + "</code>");
  // Restaurar asteriscos escapados
  text = text.replace(/\u0000ESC_AST\u0000/g, "*");
  // Restaurar imágenes
  text = text.replace(/\u0000IMG(\d+)\u0000/g, (_, i) => {
    const { alt, src } = imgs[+i];
    return '<img src="' + src + '" alt="' + escapeHtml(alt) + '"/>';
  });
  return text;
}

function convert(md) {
  const lines = md.split(/\r?\n/);
  const out = [];
  let i = 0;

  while (i < lines.length) {
    let line = lines[i];

    // Bloque de código
    if (/^```/.test(line)) {
      const buf = [];
      i++;
      while (i < lines.length && !/^```/.test(lines[i])) {
        buf.push(escapeHtml(lines[i]));
        i++;
      }
      i++; // cierre ```
      out.push("<pre><code>" + buf.join("\n") + "</code></pre>");
      continue;
    }

    // Regla horizontal
    if (/^---\s*$/.test(line)) {
      out.push("<hr/>");
      i++;
      continue;
    }

    // Encabezados
    const h = line.match(/^(#{1,6})\s+(.*)$/);
    if (h) {
      const level = h[1].length;
      out.push("<h" + level + ">" + inline(h[2]) + "</h" + level + ">");
      i++;
      continue;
    }

    // Tabla
    if (/^\|.*\|\s*$/.test(line) && i + 1 < lines.length && /^\|[\s:\-|]+\|\s*$/.test(lines[i + 1])) {
      const header = line.split("|").slice(1, -1).map((c) => c.trim());
      i += 2; // salta cabecera y separador
      const rows = [];
      while (i < lines.length && /^\|.*\|\s*$/.test(lines[i])) {
        rows.push(lines[i].split("|").slice(1, -1).map((c) => c.trim()));
        i++;
      }
      let t = "<table><thead><tr>";
      header.forEach((c) => (t += "<th>" + inline(c) + "</th>"));
      t += "</tr></thead><tbody>";
      rows.forEach((r) => {
        t += "<tr>";
        r.forEach((c) => (t += "<td>" + inline(c) + "</td>"));
        t += "</tr>";
      });
      t += "</tbody></table>";
      out.push(t);
      continue;
    }

    // Cita
    if (/^>\s?/.test(line)) {
      const buf = [];
      while (i < lines.length && /^>\s?/.test(lines[i])) {
        buf.push(inline(lines[i].replace(/^>\s?/, "")));
        i++;
      }
      out.push("<blockquote>" + buf.join("<br/>") + "</blockquote>");
      continue;
    }

    // Lista con viñetas
    if (/^\s*-\s+/.test(line)) {
      const buf = [];
      while (i < lines.length && /^\s*-\s+/.test(lines[i])) {
        buf.push("<li>" + inline(lines[i].replace(/^\s*-\s+/, "")) + "</li>");
        i++;
      }
      out.push("<ul>" + buf.join("") + "</ul>");
      continue;
    }

    // Lista numerada
    if (/^\s*\d+\.\s+/.test(line)) {
      const buf = [];
      while (i < lines.length && /^\s*\d+\.\s+/.test(lines[i])) {
        buf.push("<li>" + inline(lines[i].replace(/^\s*\d+\.\s+/, "")) + "</li>");
        i++;
      }
      out.push("<ol>" + buf.join("") + "</ol>");
      continue;
    }

    // Línea en blanco
    if (/^\s*$/.test(line)) {
      i++;
      continue;
    }

    // Párrafo (líneas consecutivas hasta blanco o bloque)
    const buf = [line];
    i++;
    while (
      i < lines.length &&
      !/^\s*$/.test(lines[i]) &&
      !/^```/.test(lines[i]) &&
      !/^#{1,6}\s/.test(lines[i]) &&
      !/^---\s*$/.test(lines[i]) &&
      !/^\|.*\|\s*$/.test(lines[i]) &&
      !/^>\s?/.test(lines[i]) &&
      !/^\s*-\s+/.test(lines[i]) &&
      !/^\s*\d+\.\s+/.test(lines[i])
    ) {
      buf.push(lines[i]);
      i++;
    }
    out.push("<p>" + inline(buf.join(" ")) + "</p>");
  }

  return out.join("\n");
}

const CSS = `
:root{ --ink:#1f2933; --muted:#52616b; --accent:#0d9488; --accent2:#0f766e; --line:#e2e8f0; --code-bg:#f4f6f8; }
*{ box-sizing:border-box; }
html{ -webkit-print-color-adjust:exact; print-color-adjust:exact; }
body{ font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;
  color:var(--ink); line-height:1.6; max-width:820px; margin:0 auto; padding:56px 40px 80px; font-size:15px; }
h1{ font-size:30px; line-height:1.25; color:var(--accent2); margin:0 0 4px; letter-spacing:-.02em; }
h2{ font-size:22px; color:var(--accent2); margin:34px 0 12px; padding-bottom:6px; border-bottom:2px solid var(--accent); }
h3{ font-size:17px; color:var(--ink); margin:24px 0 8px; }
h4{ font-size:15px; color:var(--muted); margin:18px 0 6px; text-transform:uppercase; letter-spacing:.04em; }
p{ margin:10px 0; }
strong{ color:#0b3b39; }
a{ color:var(--accent2); }
hr{ border:none; border-top:1px solid var(--line); margin:28px 0; }
ul,ol{ margin:10px 0; padding-left:24px; }
li{ margin:4px 0; }
code{ background:var(--code-bg); border:1px solid var(--line); border-radius:4px; padding:1px 5px; font-family:"SF Mono",Menlo,Consolas,monospace; font-size:.86em; }
pre{ background:#0f172a; color:#e2e8f0; border-radius:8px; padding:14px 16px; overflow:auto; font-size:12.5px; line-height:1.5; }
pre code{ background:none; border:none; padding:0; color:inherit; }
blockquote{ margin:14px 0; padding:10px 16px; background:#f0fdfa; border-left:4px solid var(--accent); border-radius:0 6px 6px 0; color:var(--muted); font-size:14px; }
img{ display:block; max-width:100%; height:auto; margin:14px auto 6px; border:1px solid var(--line); border-radius:8px; box-shadow:0 2px 10px rgba(15,23,42,.08); }
figcaption,p em:only-child{ }
table{ width:100%; border-collapse:collapse; margin:14px 0; font-size:13.5px; }
th,td{ border:1px solid var(--line); padding:7px 10px; text-align:left; vertical-align:top; }
th{ background:var(--accent2); color:#fff; font-weight:600; }
tbody tr:nth-child(even){ background:#f8fafc; }
@page{ size:A4; margin:18mm 16mm; }
@media print{
  body{ padding:0; max-width:none; font-size:11.5pt; }
  h2{ page-break-after:avoid; }
  h3,h4{ page-break-after:avoid; }
  table,pre,blockquote,img,figure{ page-break-inside:avoid; }
  tr{ page-break-inside:avoid; }
}
`;

const html = `<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1"/>
<title>TalentPact — Informe Técnico Final</title>
<style>${CSS}</style>
</head>
<body>
${convert(fs.readFileSync(SRC, "utf8"))}
</body>
</html>`;

fs.writeFileSync(OUT, html, "utf8");
console.log("Generado: " + OUT);
